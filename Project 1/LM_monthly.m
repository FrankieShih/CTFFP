...
R_LM = (log(price_LM(end)/price_LM(1))+1).^(1/10)-1;  %annualy rate real world, not for BS
...
price_LM_call = zeros(length(N),1);
...
for i = 1 : length(N)
    ...
    price_LM_call(i) = mean_discounted_payoff;
...
for i = 1 : length(N)
    ...
    %Store the results of this Monte Carlo simulation (with N = current_N)
    %to display when all MC simulations are complete. 
    price_LM_put(i) = mean_discounted_payoff;
...
price_LM_d_call = zeros(length(N),1);
...
for i = 1 : length(N)
    ...
    %Store the results of this Monte Carlo simulation (with N = current_N)
    %to display when all MC simulations are complete. 
    price_LM_d_call(i) = discounted_mean_payoff;
...
price_LM_d_put = zeros(length(N),1);
...
for i = 1 : length(N)
    ...
    %Store the results of this Monte Carlo simulation (with N = current_N)
    %to display when all MC simulations are complete. 
    price_LM_d_put(i) = discounted_mean_payoff;
